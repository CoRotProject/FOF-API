...
try:
    import CERI_AGV as robot
except:
    import CERI_AGV_Simu as robot
...
print("Factory name: ",nomfactory)

try:
    nomrobot = sys.argv[3]
except IndexError:
    nomrobot = "CERI_AGV"
print("robot name: ",nomrobot)
            ...
            if self.status == "free":
                if self.enmission: # mission en cours et free => mission finie
                    ...
                    if self.operationlist.empty(): # no pending mission
                        ...
                        self.missionid = self.connexion.go((positiontogo['x'], positiontogo['y'], positiontogo['theta'])) # specific Mir missionid
...
if __name__ == "__main__":
    ...
    robotreel = robot.CERI_AGV(nom=nomrobot)
